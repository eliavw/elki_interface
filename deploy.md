...
```bash
git remote add origin git@github.com:eliavw/elki_interface.git
git remote -v
...
```bash
git remote add origin git@github.com:eliavw/elki_interface.git; git remote -v; git push origin master
```
...
```bash
conda env create -f dependencies-deploy.yaml -n elki_interface
```
...
conda env update -n elki_interface -f dependencies-develop.yaml
```
...
```bash
conda activate elki_interface
python -m ipykernel install --user --name elki_interface --display-name "elki_interface"
```
...
1. Go to the [Travis](https://travis-ci.com/eliavw/elki_interface) page of this repo.
2. See if it ran.
...
> Very basic interface to call elki from python, cf. https://eliavw.github.io/elki_interface
